...
function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const coursResponse = await axios.get('http://localhost:8082/api/supports/type/COUR/semester/S2');
        const tdResponse = await axios.get('http://localhost:8082/api/supports/type/TD/semester/S2');
        const tpResponse = await axios.get('http://localhost:8082/api/supports/type/TP/semester/S2');
                  ...
                  <ListItem key={index} secondaryAction={
                    <Stack direction="row" spacing={1}>
                      <Button
                        ...
                        onClick={() => handleDownloadClick(item.id, item.nomModule)}
                      >
                        Télécharger
                      </Button>
                      <Button
