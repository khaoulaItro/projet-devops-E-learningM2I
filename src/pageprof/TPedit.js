...
function TPedit() {
    ...
    const myFormik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                ...
                // Étape 3 : Mise à jour des données de TPs
                await axios.put(
    ...
    const getTPsData = async () => {
        try {
            ...
            const tpsData = response.data;

            // Assurez-vous que les données correspondent aux initialValues
            myFormik.setValues({
                idProfesseur: tpsData.idProfesseur || "",
                description: tpsData.description || "",
                nomModule: tpsData.nomModule || "",
                semester: tpsData.semester || "",
                codeClassroom: tpsData.codeClassroom || "",
                type: tpsData.type || "",
                fichierUrl: tpsData.fichierUrl || "",
            });
            setLoading(false);
        } catch (error) {
    ...
    return (
        <>
            <h3>Modification du TP - ID : {params.id}</h3>
